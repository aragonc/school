...
<div class="content">
    <p><strong>OTEC EDUCHILE</strong> certifica que <strong>{{ data.firstname }} {{ data.lastname }}</strong>, con RUN <strong>{{ data.rut }}</strong>,
        se encuentra como alumno regular del Programa: <strong>{{ data.course_name }}</strong> con
        fecha de inicio: <strong>{{ data.display_start_date }}</strong> y término <strong>{{ data.display_end_date }}</strong>.</p>
...
<div class="firma-block" style="text-align: center;">
    <div>
        ...
        <div style="text-align: center; width: 400px;  border-top: 1px solid #000; display: inline-block; margin: auto;">
            <strong>OTEC EDUCHILE</strong><br>
            Pablo Rodríguez<br>
            Director Académico
        </div>
    </div>
